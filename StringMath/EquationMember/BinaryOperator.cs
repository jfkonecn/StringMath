    ...
    OperatorAssociativity associativity, Func<double, double, double> evaluator)
        ...
        public static readonly List<BinaryOperator> AllOperators =
            new List<BinaryOperator>()
            {
                ...
                new BinaryOperator(@"^\s*[-−]", 2, OperatorAssociativity.LeftAssociative,
                    (double left, double right)=>{ return left - right; }),
                new BinaryOperator(@"^\s*&&", 0, OperatorAssociativity.LeftAssociative,
                    (double left, double right)=>{ return Convert.ToDouble(Convert.ToBoolean(left) && Convert.ToBoolean(right)); }),
                new BinaryOperator(@"^\s*\|\|", 0, OperatorAssociativity.LeftAssociative,
                    (double left, double right)=>{ return Convert.ToDouble(Convert.ToBoolean(left) || Convert.ToBoolean(right)); }),
                new BinaryOperator(@"^\s*==", 1, OperatorAssociativity.LeftAssociative,
